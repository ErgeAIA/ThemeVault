# tokyonight/storm 色板表

> 来源：tokyonight.nvim（folke）`lua/tokyonight/colors/storm.lua`（完整基础 palette，night 由其派生）。
...
| 语义角色 | 颜色值 | 类型 | 备注 |
|----------|--------|------|------|
| `--bg` | `#24283b` | 中性 | bg，storm 变体 |
| `--bg-surface` | `#1f2335` | 中性 | bg_dark（侧栏/状态栏） |
| `--bg-surface-2` | `#1b1e2d` | 中性 | bg_dark1 |
| `--bg-elevated` | `#24283b` | 中性 | bg（浮窗背景与 bg 同色） |
| `--bg-hover` | `#292e42` | 中性 | bg_highlight（highlight 背景） |
| `--text` | `#c0caf5` | 中性 | fg |
...
| `--accent-foreground` | `#24283b` | 强调 | bg（accent 上文字用深背景） |
