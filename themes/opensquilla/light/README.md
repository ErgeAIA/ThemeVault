...
- **提取方式**：基于真实源码 `opensquilla-webui/src/themes/light/tokens.css`
- **显式颜色令牌数**：59（含结构/焦点环等非纯色令牌）

## 迁移备注

- 目标项目命名对齐见 `docs/migration-guide.md` §4 映射矩阵。
- 浅色 baseline 优先取 `light`，深色取 `dark`；其余 7 套作风格化皮肤叠加。
- 特殊点：Lark 结构逻辑 + 暖橙品牌；含完整 sidebar 派生覆盖 + 浅色阴影 + atmosphere 渐变
