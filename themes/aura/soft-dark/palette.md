...
> 来源：aura-theme `src/core/colors/schemes/soft-dark.ts` + `common.ts` + VSCode `aura-soft-dark-color-theme.json`（MIT）。
...
| 语义角色 | 颜色值 | 类型 | 备注 |
|----------|--------|------|------|
| `--bg` | `#21202e` | 中性 | Aura accent12（editor.background；soft 背景） |
| `--bg-surface` | `#1f1a27` | 中性 | accent24（titleBar/statusBar/editorWidget） |
| `--bg-surface-2` | `#2e2b38` | 中性 | accent33（list.activeSelectionBackground） |
| `--bg-elevated` | `#1c1b22` | 中性 | accent21（sideBar；Aura 侧栏比 bg 更暗） |
...
| `--border-strong` | `#141414` | 中性 | accent11（panel/titleBar/tab.border） |
| `--border-focus` | `#a277ff` | 中性 | accent1（inputOption.activeBorder；focusBorder 本体为 accent17 半透明） |
| `--card` | `#1f1a27` | 中性 | accent24（editorWidget.background） |
| `--hairline` | `#444444` | 中性 | accent13（editorWidget.border） |
...
| `--accent-foreground` | `#21202e` | 强调 | accent12（badge/button 上文字；soft 背景版） |
